...
export const sortByNames = (n1: string, n2: string, desc: boolean = false) => {
	let result: number = 0;
	if (desc) {
		if (n1 > n2) {
			result = -1;
...
export const sortByIds = (n1: any, n2: any, desc: boolean = false) => {
	let result: number = 0;
	if (desc) {
		if (+n1 > +n2) {
			result = -1;
...
export const numberToLatter = (number: number, uppercase?: boolean): string => {
