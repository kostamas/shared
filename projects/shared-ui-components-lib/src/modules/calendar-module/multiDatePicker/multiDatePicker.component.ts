...
export class MultiDatePickerComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		if (this.config && this.config.selectedRange) {
			setTimeout(() => {
				...
				this.calendarDatePickerService.selectedRange = {date1: moment(firstDate, DATE_FORMAT), date2: moment(lastDate, DATE_FORMAT)};
	...
	rangeFromInputHandler(): void {
		...
		this.calendarDatePickerService.selectedRange = {date1: moment(newFrom, 'DD/MM/YYYY'), date2: moment(newTo, 'DD/MM/YYYY')};
